...
| [`08_Lasso.ipynb`](./08_Lasso.ipynb) | **L1 regularisation and feature selection.** Lasso adds `α × \|slope\|` penalty to the cost function. Unlike Ridge, Lasso subtracts penalty directly from the slope, allowing slopes to reach exactly `0` for feature selection. Demonstrates how increasing `alpha` systematically reduces coefficients and flattens the fitted regression line. |
| [`09_ElasticNet.ipynb`](./09_ElasticNet.ipynb) | **Combining Ridge (L2) and Lasso (L1).** ElasticNet blends L1 and L2 penalties controlled by `alpha` (penalty strength) and `l1_ratio` (mix ratio). Demonstrates how tuning `alpha` from `0.01` to `100` shifts the model from unregularized Linear Regression to a flat horizontal mean-line. |
