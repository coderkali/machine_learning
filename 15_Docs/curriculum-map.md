...
|---|---|---:|---|---|---|---|
| M01 | Python | 3-5 | 01 — Engineering and Python foundation | Syntax, collections, control flow, functions, arrays, OOP, files, exceptions, packages, environments | 🟡 Learning | Current: [Session 01 notebook and cell-by-cell guide](../01_Python/01_Variables_And_Data_Types/Concept/README.md), covering variables, data types, numbers, strings, input, conversion, and formatted output. This is partial M01 evidence, not module completion. |
| M02 | Data Visualization with Python | 5-6 | 02 — Python data toolkit, visualization, and rapid apps | Matplotlib, Seaborn, Plotly, static and interactive charts | 🟡 Learning | Current: [Matplotlib example notebook](../01_Python/04_Matplotlib/Concept/example_01.ipynb), documented in its own Markdown cells, covering line, scatter, bar, histogram, pie, fill, annotation, subplots, and `savefig`; and [Seaborn example notebook](../01_Python/05_Seaborn/Concept/example_01.ipynb) with [topic README](../01_Python/05_Seaborn/Concept/README.md), covering relational, distribution, categorical, regression, grid, and styling plots against seaborn 0.13.2; and [Plotly example notebook](../01_Python/06_Plotly/Concept/example_01.ipynb) with [topic README](../01_Python/06_Plotly/Concept/README.md) and a [3D projection concept note](../01_Python/06_Plotly/Concept/concept_3d_projection.md), covering line, scatter, bar, box, violin, pie, area, 3D scatter, and sunburst against plotly 6.9.0. Prior only: [Matplotlib](../01_Python/04_Matplotlib/Concept/example_01.py), [pizza plotting](../01_Python/04_Matplotlib/Concept/pizza_matplotlib_2.ipynb), and [Seaborn](../01_Python/05_Seaborn/Concept/pizza_seaborn.ipynb) |
| M03 | Python for Data Science (NumPy & Pandas) | 6-8 | 02 — Python data toolkit, visualization, and rapid apps | NumPy arrays, operations and linear algebra; Pandas I/O, cleaning, selection, transformation, grouping, joins, pivots, dates, and strings | 🟡 Learning | Current: [NumPy example 01 — arrays](../01_Python/02_NumPy/Concept/example_01.ipynb), [NumPy example 02 — operations, broadcasting, and linear algebra](../01_Python/02_NumPy/Concept/example_02.ipynb), [NumPy topic README](../01_Python/02_NumPy/Concept/README.md), [partial API-health project](../01_Python/02_NumPy/Concept/project_01_api_health.ipynb), [Pandas example 01 — shapes and first operations](../01_Python/03_Pandas/Concept/example_01.ipynb), [Pandas example 02 — the working toolkit](../01_Python/03_Pandas/Concept/example_02.ipynb), and the [Pandas topic README](../01_Python/03_Pandas/Concept/README.md). The project has only Parts 1-2 of 12, and two I/O cells in Pandas example 02 do not run. Prior only: [NumPy](../01_Python/02_NumPy/Concept/example_02.py), [Pandas](../01_Python/03_Pandas/Concept/example_01.py), [pizza NumPy](../01_Python/02_NumPy/Concept/pizza_numpy.ipynb), and [pizza Pandas](../01_Python/03_Pandas/Concept/pizza_panda.ipynb) |
| M04 | Streamlit | 8-9 | 02 — Python data toolkit, visualization, and rapid apps | UI elements, charts, inputs, media, chat, containers, state, multipage apps, GitHub deployment, secrets | 🟡 Learning | Started: [`07_streamlit/`](../01_Python/07_Streamlit) with [topic README](../01_Python/07_Streamlit/Concept/README.md) and a [Tips Explorer app](../01_Python/07_Streamlit/Content/app.py) covering widgets, layout, caching, session state, and a Community Cloud deployment plan. Learner-written evidence: [Sales Analysis app](../01_Python/07_Streamlit/Content/sales_analysis.py) with a [line-by-line review](../01_Python/07_Streamlit/Concept/sales_analysis_explained.md); two re-run/session-state defects are open and tracked in the review queue. |
| M05 | Statistics and Probability | 9-10 | 03 — Math and relational-data foundations | Descriptive statistics, probability, distributions, inference, tests, correlation, information theory | ⬜ Not Started | Prior only: [statistics notes](../03_Math/02_Statistics/Concept) and [hypothesis-testing notebooks](../02_DataScience) |
| M06 | Linear Algebra | 10-11 | 03 — Math and relational-data foundations | Scalars, vectors, matrices, norms, similarity, eigenvalues, PCA, LU, QR, SVD | ⬜ Not Started | Prior only: [linear-algebra archive](../03_Math/03_Linear_Algebra) |
| M07 | Machine Learning | 11-14 | 05 — Classical modeling and forecasting | Data loading, EDA, preprocessing, validation, feature work, regression, classification, clustering, ensembles, evaluation, tuning, saving, deployment | 🟡 Learning | Current: seven documented notebooks in [`08_sklearn/`](sklearn_module_readme.md) — [`train_test_split`](../04_ML/13_Train_Test_Split/Concept/01_train_test_split_data.ipynb), [synthetic datasets](../04_ML/14_Synthetic_Datasets/Concept/02_make_*_dataset.ipynb), [preprocessing](../04_ML/12_Preprocessing/Concept/03_preprocessing.ipynb), [EDA](../04_ML/15_EDA_Uni_Bi_Multivariate/Concept/04_uni_boi_multi_variate_analaysis.ipynb), [linear regression](../04_ML/16_Linear_Regression/Concept/05_Linear_Regression.ipynb), [multiple linear regression](../04_ML/17_Multiple_Linear_Regression/Concept/06_multiple_linear_regression.ipynb), and [Ridge](../04_ML/19_Ridge_Regression/Concept/07_Ridge.ipynb), all re-run end to end. Learner-written evidence: `X`/`Y` selection and a full `train_test_split` call, documented parameter by parameter with three verified experiments and three charts covering `test_size`, `shuffle`, `random_state`, `stratify`, and the four-value return order; plus all five dataset generators — `make_regression` (`noise`, `bias`, `coef`, `n_informative`), `make_classification` (informative / redundant / useless column anatomy, `class_sep`, `flip_y`, `weights`, `n_clusters_per_class`, column shuffling), `make_blobs` (`cluster_std`, explicit `centers`), `make_circles` (`factor`), and `make_moons` — with 24 verified charts, a measured straight-line ceiling for each shape, and the learner's own `IndexError` explained rather than removed. Preprocessing adds the three scalers hand-checked against sklearn, `Binarizer`, `Normalizer`, `LabelEncoder`, `OneHotEncoder`, a measured leakage demo, and a first `Pipeline` + `ColumnTransformer`. **The first estimators are now fitted:** `LinearRegression` on one input column and then on two, covering `fit`/`predict`/`score`, R² read honestly, `coef_` and `intercept_` as the entire model, `predict()` rebuilt by hand and verified with `np.allclose`, the line-to-plane jump drawn in 3-D, coefficients as partial effects, and a first measured case of multicollinearity. **Regularisation has a first pass:** `Ridge` as `error + α × slope²`, `alpha` named as a hyperparameter rather than a learned value, the one-column shrink factor `new slope = old slope × S/(S+α)` verified against every printed coefficient with `np.allclose` across nine alphas, why a large `alpha` rotates the fitted line flat onto `mean(y_train)` and drives R² to 0 (the intercept is not penalised), and an alpha table from `0` to `10,000,000`. The notes are deliberately kept short at the learner's request — the matrix form `(XᵀX + αI)⁻¹Xᵀy`, `RidgeCV`, the scaling requirement, and a worked overfitting case are all left for a later pass. **Still open in M07:** every algorithm outside the linear family, Lasso and ElasticNet, every metric beyond `.score()`, `cross_val_score`, `RidgeCV` and `GridSearchCV`, and persistence. The instructor introduced scikit-learn during Phase 02, so the folder sits in `02_data_toolkit_apps/08_sklearn/` even though M07 maps to dependency phase 05. Prior only: [ML archive](../04_ML) and [classical-ML archive](../04_ML); the archive's [attrition project](../04_ML/35_Project_Employee_Attrition/Concept/Employee_Attrition_Pipeline.ipynb) fits preprocessing before splitting, which the review queue tracks as an open leakage defect |
...
| M27 | MLOps | 50-51 | 10 — Containers and ML operations | Lifecycle, environments, Git/DVC, model persistence and serving, Docker, MLflow tracking and registry, drift monitoring | ⬜ Not Started | Prior only: [employee-attrition pipeline](../04_ML/35_Project_Employee_Attrition); it is data-workflow evidence, not MLOps completion |
...
| M39 | System Design for AI Systems | 66-67 | 13 — AI system design and career proof | Scale, reliability, latency, consistency, building blocks, microservices, data/ML/LLM/RAG/vector architecture, security and cost | ⬜ Not Started | Prior only: [one pipeline architecture](../04_ML/35_Project_Employee_Attrition/Concept/ARCHITECTURE.md); it is not full AI-system-design evidence |
...
[`02_data_toolkit_apps/08_sklearn/`](sklearn_module_readme.md)
